...
install: deps
	...
	@# `make install` installs the editable version of openfisca-taiwan.
...
build: clean deps
	...
	python -m build
	pip uninstall --yes openfisca-taiwan
...
test: clean
	openfisca test --country-package openfisca_taiwan openfisca_taiwan/tests

serve-local: build
	openfisca serve --country-package openfisca_taiwan
